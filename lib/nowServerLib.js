  ...
  constructClientScopeStore: function(client) {
    return {
      set: function(key, val, callback, changes){
        ...
        var data =  nowUtil.decycle(val, "now."+key, [nowUtil.serializeFunction, everyonegenerateMultiCaller]);
        // data[0] = For client
        // data[1] = For Everyone
...
exports.getGroup = function(groupName){
    if(!nowCore.groups.hasOwnProperty(groupName)){
      ...
      nowCore.groups[groupName] = new ClientGroup(scopes, nowCore.defaultScopes[groupName], socket);
    }
...
exports.initialize = function(server, options){
  ...
  everyone = new ClientGroup(nowCore.scopes, nowCore.defaultScopes.everyone, socket, nowCore.groups, nowCore.clientGroups);
