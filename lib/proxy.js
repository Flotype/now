    ...
    return Proxy.create({
      get : function (receiver, name) {
        ...
        if (returnObj && typeof returnObj === 'object') {
          ...
          var p = !returnObj.filter || returnObj.filter(isNaN).length ? Object.prototype : Array.prototype;
          if (p === Array.prototype) {
            entity.scopeTable.flagAsArray(fqn, returnObj[returnObj.length - 1] || 0);
          }
          return (entity.proxies[fqn] = wrapRoot(fqn, p));
        }
      ...
      set : function (receiver, name, value) {
        ...
        if (len !== undefined) {
          if (name === 'length') {
            return (entity.scopeTable.arrays[path] = value);
          }
          while (len < name) {
