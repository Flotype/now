(function () {
  ...
  var fqnMap = {
    ...
    set: function (fqn, val) {
      ...
      if (parent && !util.hasProperty(fqnMap.data, parent)) {
        fqnMap.set(parent, []);
      }
  ...
  var lib = {
    ...
    processScope: function () {
      var data = {};
      lib.traverseScope(now, 'now', data);
      ...
      // customizable, perhaps? Or possibly more intelligent calling?
      setTimeout(lib.processScope, 1000);
    },
          ...
          if (util.hasProperty(obj, key)) {
            if ((isIE || objIsArray) &&
              ...
              if (fqnMap.get(fqn) === undefined) {
                var val = obj[key]; // keep some reference to this value.
                fqnMap.set(fqn, obj[key]);
                if (type === 'function') {
                  if (obj[key].remote) {
                    continue;
