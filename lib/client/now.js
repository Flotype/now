(function(){
  ...
  var util = {
    ...
    emit: function (name, args){
      if (name in util._events){
        var events = util._events[name].concat();
  ...
  var lib = {
    createScope: function(data) {
      ...
      watcher = new nowLib.NowWatcher('now', now, {}, function (prop, fqn, oldVal, newVal){
        ...
        if(!util.hasProperty(blacklist, fqn)){
          ...
          client.emit('rv', {fqn: fqn, val: newVal});
        } else {
    ...
    replaceVar: function(data) {
      blacklist[data.fqn] = true;
      util.createVarAtFqn(data.fqn, now, data.val);
    },
    ...
    remoteCall: function(data) {
      ...
      for(var i = 0, ii = args.length; i < ii; i++){
        if(util.hasProperty(args[i], 'type') && args[i].type === 'function'){
