...
exports.initialize = function (nowjs) {
  ...
  var User = function (socket) {
    ...
    /* Load a reference to the session provided by the Connect middleware
     *  (if present) in this.user.session */
    console.log(nowjs.options.connect);
    if (nowjs.options.connect.store && typeof nowjs.options.connect.store === 'object') {
      console.log("OBJECT");
      this.user.session = nowjs.options.connect.store.sessions[this.user.cookie[nowjs.options.connect.key]];
    }
