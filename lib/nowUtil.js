...
var nowUtil = {
  ...
  multiForceGetParentVarAtFqn: function(fqn, scopes){
    ...
    while(path.length > 1){
      var prop = path.shift();
      for(var i in scopes) {
        if(!nowUtil.hasProperty(currVar[i], prop)){
          if(!isNaN(path[0])) {
            currVar[i][prop] = [];
          } else {
            currVar[i][prop] = {};
    ...
    for(var prop in incoming){
      if(nowUtil.hasProperty(incoming, prop)){
        if(incoming[prop] && typeof incoming[prop] === "object"){
          ...
          for(i in current) {
            if(nowUtil.hasProperty(current, i)) {
              var curItem = current[i][key];        
              if(!nowUtil.hasProperty(curItem, prop)){
                if(nowUtil.isArray(incoming[prop])) {
                  curItem[prop] = [];
                } else {
                  curItem[prop] = {};
  ...
  shallowCopy: function(incoming) {
    if(nowUtil.isArray(incoming)) {
      ...
      for(var key in incoming) {
        if(nowUtil.hasProperty(incoming, key)) {
        ...
        case 'object':
          ...
          if (Object.prototype.toString.apply(value) === '[object Array]') {
            ...
            for (i = 0; i < value.length; i += 1) {
              ...
              for(j in values) {
                if (nowUtil.hasProperty(values, j)) {
                  nu[j][i] = values[j];
                }
              }
            }
            ...
            for (var name in value) {
              if (nowUtil.hasProperty(value, name)) {
    ...
    (function rez(value) {
        ...
        if (value && typeof value === 'object') {
            if (Object.prototype.toString.apply(value) === '[object Array]') {
                for (i = 0; i < value.length; i += 1) {
                    item = value[i];
                    if(nowUtil.hasProperty(item, "type") && item.type === 'function') {
  ...
  mapAndMergeFunctionsInScopesHelper: function(target, incoming, mapFn, fqn){
    for(var key in incoming){
      fqn += '.' + key;
      if(target && !nowUtil.hasProperty(target, key)){
...
if('window' in this) {
  ...
  if(!('JSON' in window)){
    ...
    if(rep&&typeof rep==='object'){length=rep.length;for(i=0;i<length;i+=1){if(typeof rep[i]==='string'){k=rep[i];v=str(k,value);if(v){partial.push(quote(k)+(gap?': ':':')+v);}}}}else{for(k in value){if(nowUtil.hasProperty(value,k)){v=str(k,value);if(v){partial.push(quote(k)+(gap?': ':':')+v);}}}}
    v=partial.length===0?'{}':gap?'{\n'+gap+partial.join(',\n'+gap)+'\n'+mind+'}':'{'+partial.join(',')+'}';gap=mind;return v;}}
    if(typeof JSON.stringify!=='function'){JSON.stringify=function(value,replacer,space){var i;gap='';indent='';if(typeof space==='number'){for(i=0;i<space;i+=1){indent+=' ';}}else if(typeof space==='string'){indent=space;}
    rep=replacer;if(replacer&&typeof replacer!=='function'&&(typeof replacer!=='object'||typeof replacer.length!=='number')){throw new Error('JSON.stringify');}
    return str('',{'':value});};}
    if(typeof JSON.parse!=='function'){JSON.parse=function(text,reviver){var j;function walk(holder,key){var k,v,value=holder[key];if(value&&typeof value==='object'){for(k in value){if(nowUtil.hasProperty(value,k)){v=walk(value,k);if(v!==undefined){value[k]=v;}else{delete value[k];}}}}
