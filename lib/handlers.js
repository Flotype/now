...
exports.initialize = function (nowjs) {
  ...
  nowjs.on('removegroup', function (e, args) {
    ...
    if (nowUtil.hasProperty(this.groups, name)) {
      ...
      this.getGroup(name, function () {
        var group = this;
        // so we can see its members
        this.getUsers(function (users) {
          users.map(function (user) {
            // and remove them all from the group.
            self.getClient(user, function () {
