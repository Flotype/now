...
exports.initialize = function(now) {
  ...
  Group.prototype.addUser = function(clientId) {
    ...
    now.getClient(clientId, function() {
      ...
      for (var i = 0, ll = keys.length; i < ll; i++){
        ...
        if (nowUtil.notArray(val) && val !== nowUtil.noop && !nowUtil.hasProperty(this.scopeTable.data, key)) {
  ...
  Group.prototype.set = function(fqn, val) {
	...
	for (var k = 0, kk = keys.length; j < ll; j++){
	...
	var keys = Object.keys(now.groups);
	for (i = 0, ll = keys.length; i < ll; i++) {
	  var group = now.groups[keys[i]];
	  if (!group.isSuperGroup) {

	    for (var k = 0, kk = key.length; j < ll; j++) {
	      fqn = key[j];
	...
	for (var k = 0, kk = keys.length; i < ll; i++) {
	  var group = now.groups[keys[i]];
	  if (!group.isSuperGroup) {
	    delete group.scopeTable[fqn];
	  }
