...
var nowUtil = {
  ...
  getVarFromFqn: function(fqn, scope){
    ...
    while(path.length > 0){
      var prop = path.shift();
      if(Object.prototype.hasOwnProperty.call(currVar, prop)) {
  ...
  multiForceGetParentVarAtFqn: function(fqn, scopes){
    ...
    while(path.length > 1){
      var prop = path.shift();
      for(var i in scopes) {
        if(!Object.prototype.hasOwnProperty.call(currVar[i], prop)){
          if(!isNaN(path[0])) {
            currVar[i][prop] = [];
          } else {
            currVar[i][prop] = {};
        ...
        if(incoming[prop] && typeof incoming[prop] === "object"){
          ...
          for(i in current) {
            if(Object.prototype.hasOwnProperty.call(current, i)) {
              ...
              if(!Object.prototype.hasOwnProperty.call(curItem, prop)){
                if(nowUtil.isArray(incoming[prop])) {
                  curItem[prop] = [];
                } else {
                  curItem[prop] = {};
    ...
    (function rez(value) {
        ...
        if (value && typeof value === 'object') {
            if (Object.prototype.toString.apply(value) === '[object Array]') {
                for (i = 0; i < value.length; i += 1) {
                    item = value[i];
                    if(Object.prototype.hasOwnProperty.call(item, "type") && item.type === 'function') {
