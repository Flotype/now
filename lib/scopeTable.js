...
ScopeTable.prototype.set = function (fqn, val) {
  ...
  if (parent != "now" && !Object.prototype.hasOwnProperty.call(this.data, parent)) {
    this.set(parent, []);
  }
  if (this.data[fqn] === undefined)
    this.data[parent].push(fqn.substring(lastIndex));
...
ScopeTable.prototype.delete = function (fqn) {
  ...
  if (this.data[parent]) {
    this.data[parent].splice(this.data[parent].indexOf(fqn.substring(lastIndex)), 1);
  }
