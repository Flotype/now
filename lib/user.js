...
exports.initialize = function (nowjs) {
  ...
  var User = function (socket) {
    ...
    socket.on('rfc', function rfcHandler(data) {
      if (!(nowUtil.hasProperty(data, 'fqn'))){
        nowUtil.error("missing fqn in remote function call");
        return
      } else if (!(nowUtil.hasProperty(data, 'args'))) {
        nowUtil.error("missing args in remote function call");
        return
      }    
      ...
      try {
        ...
        var user;
        //check if the length of args is too long before the loop
        if (args.length > 1e7) {
          throw new Error("length of args too long");
        }
      ...
      try {
        ...
        console.log("error while calling the remote function");
        nowUtil.error(err);
        return;  
      }
    });
    ...
    if (nowjs.options.clientWrite) {
      socket.on('rv', function rvHandler(data) {
        try {
          //catch the error when the data is a non-object
          var keys = Object.keys(data);
        } catch (err) {
          nowUtil.error("invalid input in the event Replace Var");
          nowUtil.error(err);
          return;
        }
