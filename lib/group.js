...
exports.initialize = function (now) {
  ...
  Group.prototype.set = function (fqn, val) {
    var exclusive = Object.keys(this.excludes).length !== 0;
    var keys = Object.keys(this.users);
    ...
    if (typeof val === 'object') {
      ...
      if (!exclusive) {
        // Not storing a reference because Object.keys does not
        // include properties of the prototype.
        this.scopeTable.set(fqn, Object.keys(val));
      }

      // Recurse for all children
      keys = Object.keys(flattenedVal);
      for (ll = keys.length; i < ll; i++) {
        fqn = keys[i];
        this.set(fqn, flattenedVal[fqn]);
        // Send the serialized form of functions.
        flattenedVal[fqn] = nowUtil.getValOrFqn(flattenedVal[fqn], fqn);
      }
