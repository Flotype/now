...
now.core.init = function(host, port, dependencies, socketOptions){
  ...
  //set default socket options and set port
  if (!socketOptions) socketOptions = {};
  ...
  var nowJSScriptLoaded = function(){
    ...
    socket = new io.Socket(host, socketOptions);
  ...
  for(var i=0, ii=dependencies.length; i < ii; i++){
    ...
    fileref.setAttribute("src", "http://" + host + ":" + port + dependencies[i]['path']);
...
if (!window.__NOWMANUALINIT) now.core.init();
