var nowUtil = require('./nowUtil');

exports.init = function(nowjs) {
  return {
    multicall = function(){
      for (var i = 0, k = Object.keys(this.group.users), l = k.length; i < l; i++){
        var user = this.group.users[k[i]];
        var func = user.get(this.fqn);
        
        // Dirty hack to add fqn to user object. This shouldn't be problem in single threaded sync environment
        user.fqn = this.fqn;
    ...
    remotecall = function(){
      ...
      this.user.socket.emit('rfc', {fqn: this.fqn, args: args});
    };
  };
};