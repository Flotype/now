      ...
      for(var i in targetScopes){
        ...
        if(target && !target.hasOwnProperty(key)){
          if(nowUtil.isArray(incomingScope[key])){
            ...
            nowUtil.multiMergeFunctionsToMulticallersHelper(targetGroups, targetScopes, incomingScope[key], fqn);
          } else if (typeof incomingScope[key] == 'object'){
            target[key] = {};
            targetScopes[i] = target[key];
            nowUtil.multiMergeFunctionsToMulticallersHelper(targetGroups, targetScopes, incomingScope[key], fqn);
          } else if(typeof incomingScope[key] == 'function'){
            target[key] = targetGroups[i].generateMultiCaller(fqn);
          }
        }
        console.log(target);
        console.log(targetScopes[i]);
      }
